 ...
 * @author renfei
 */
public class SM2X509CertFactory extends AbstractSM {
    ...
    private final List<GeneralName> subjectAltNames = new LinkedList<>();
    ...
    private JcaContentSignerBuilder makeContentSignerBuilder(PublicKey issPub) {
        ...
        if (issPub.getAlgorithm().equals(EC_VALUE)) {
            contentSignerBuilder = new JcaContentSignerBuilder(SM3SM2_VALUE);
        ...
        for (RDN rdn : rdns) {
            ...
            if (BCStyle.EmailAddress.equals(type)) {
                ...
                if (BCStyle.CN.equals(type)) {
                    String commonName = IETFUtils.valueToString(atv.getValue());
                }
                newRdns.add(rdn);
            }
        }
